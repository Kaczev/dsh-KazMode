			...
			function RoundDisplayPanel({ sessionId, onClose }) {
				...
				return createElement(
					...
					createElement(
						"p",
						{ className: "rd-note" },
						"Kaz 模式联动/附属插件在本轮（每次用户消息 = 一轮）给模型发送的信息，含 kaz-system-prompt 上报的真实系统提示词（plan 模式时含 plan:policy 段；goal 工具段启用时含 tool:goal 段）、plan-mode 进入/退出通知、goal-round-driver 的 <goal_round> 与 tool-goal 的 <goal_complete>/<goal_blocked>，以及 round-minimal 上报的首轮工具提示与工具面增删明细。格式：[插件名]>（信息内容）<。",
