...
check("① worker 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.worker.includes("We execute one delegated plan item with care and precision") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Worker flow: challenge-plan → working → compress_context_then_communication.") && !KAZ_ROLE_PROMPTS.subagent.worker.includes("Worker flow: assess-complexity") && !KAZ_ROLE_PROMPTS.subagent.worker.includes("compass_context") && !KAZ_ROLE_PROMPTS.subagent.worker.includes("terminal communication") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Each *_sub_whale_report is the single settled channel") && KAZ_ROLE_PROMPTS.subagent.worker.includes("at terminal compress_context_then_communication after the terminal full report, the next parent message starts a fresh delegation.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("We do not write memories or private plugins ourselves.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Manage context proactively: use context_search/read"));
check("① memoryMaintainer 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("We need to maintain project memories with evidence, keep new entries as CANDIDATE, delete only explicitly listed items with an audit record.") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Flow: plan-memory → save-update or delete-memory → compress_context_then_communication.") && !KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("assess-delegation") && !KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("compass_context") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("call memory_sub_whale_report({nextStage:'compress_context_then_communication'}) with an intermediate") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("write the FULL final report as your final message") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("The parent receives it as subagent-settled") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("at terminal compress_context_then_communication after the terminal full report") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("The same memoryMaintainer sub-agent can be reused multiple times; each round starts with the \"plan-memory\" process") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Manage context proactively: use context_search/read"));
check("① pluginMaintainer 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("We need to maintain private plugins with CANDIDATE → implementation → probe → registration/versioning discipline") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Flow: plan-plugin → create-plugin or update-plugin or retire-plugin → compress_context_then_communication.") && !KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("assess-delegation") && !KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("compass_context") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("call plugin_maintainer_sub_whale_report({nextStage:'compress_context_then_communication'}) with an intermediate") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("write the FULL final report as your final message") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("The parent receives it as subagent-settled") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("at terminal compress_context_then_communication after the terminal full report") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Whether to reuse is determined by the main agent: messages can be sent directly to the same \"surface + idle child\"") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Manage context proactively: use context_search/read"));
