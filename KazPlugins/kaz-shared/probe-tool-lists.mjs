...
check("① MANAGED_CARRIER_TOOLS 覆盖 v0.9 workflow 与 kaz-context-policy 工具，MANAGED_PLUGINS 无 create-plan", MANAGED_CARRIER_TOOLS["ka-whale-workflow"]?.includes("whale_report") === true && MANAGED_CARRIER_TOOLS["ka-whale-workflow"]?.includes("ka_sub_whale") === true && JSON.stringify(MANAGED_CARRIER_TOOLS["kaz-context-policy"]) === JSON.stringify(["context_search", "context_read", "context_compress"]) && MANAGED_CARRIER_TOOLS["create-plan"] === undefined && !MANAGED_PLUGINS.some((p) => p.id === "create-plan"));
