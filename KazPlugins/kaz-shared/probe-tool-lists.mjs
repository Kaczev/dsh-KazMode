...
check("① main 含 context 纪律 + keep-gray/stop（7.4 新口径）", KAZ_ROLE_PROMPTS.main.includes("Manage context proactively; before a long session closes") && KAZ_ROLE_PROMPTS.main.includes("preview with context_compress suggest, then fold when large") && KAZ_ROLE_PROMPTS.main.includes(KAZ_PROMPT_PHRASES.keepGray) && KAZ_ROLE_PROMPTS.main.includes("If stuck or circling, report and stop."));
