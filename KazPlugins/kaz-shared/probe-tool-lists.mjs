...
check("① main 含角色流程一句 + relay/复用口径", KAZ_ROLE_PROMPTS.main.includes("We drive the ka-whale-workflow run and verify delegated reports.") && KAZ_ROLE_PROMPTS.main.includes("Main flow: assess-complexity → challenge-plan → decide-tools → write-plan → working → memory-maintenance → plugin-maintenance → compass_context (optional context tidy) → communication.") && KAZ_ROLE_PROMPTS.main.includes("Advance with whale_report and follow the injected [ka-whale-workflow <stage>] body each turn.") && KAZ_ROLE_PROMPTS.main.includes("reuse idle children by default") && KAZ_ROLE_PROMPTS.main.includes("planItemId: <id>"));
check("① main 含 context 纪律 + keep-gray/stop（7.4 新口径）", KAZ_ROLE_PROMPTS.main.includes("Manage context proactively; before a long session closes") && KAZ_ROLE_PROMPTS.main.includes("preview with context_compress suggest, then fold when large") && KAZ_ROLE_PROMPTS.main.includes("Keep gray reasoning concise — short English sentences.") && KAZ_ROLE_PROMPTS.main.includes("If stuck or circling, report and stop."));
