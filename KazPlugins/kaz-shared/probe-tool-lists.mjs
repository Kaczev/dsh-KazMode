...
check("① v0.9 Stable Main Surface = 21（§1.1，M3.2 含 context 两工具，无 create_goal/subagent）", KAZ_STABLE_MAIN_TOOLS.length === 21 && KAZ_STABLE_MAIN_TOOLS.every((t) => typeof t === "string") && KAZ_STABLE_MAIN_TOOLS.includes("context_read") && KAZ_STABLE_MAIN_TOOLS.includes("context_search") && KAZ_STABLE_MAIN_TOOLS.includes("whale_report") && KAZ_STABLE_MAIN_TOOLS.includes("ka_sub_whale") && KAZ_STABLE_MAIN_TOOLS.includes("list_agents") && KAZ_STABLE_MAIN_TOOLS.includes("send_message") && KAZ_STABLE_MAIN_TOOLS.includes("interrupt_agent") && !KAZ_STABLE_MAIN_TOOLS.includes("create_goal") && !KAZ_STABLE_MAIN_TOOLS.includes("subagent"));
check("① stableMainSurface = v0.9 固定集 21", stableMainSurface().size === 21 && stableMainSurface().has("context_read") && stableMainSurface().has("context_search") && stableMainSurface().has("ka_sub_whale") && stableMainSurface().has("get_goal") && stableMainSurface().has("update_goal") && !stableMainSurface().has("exit_plan_mode") && !stableMainSurface().has("subagent") && !stableMainSurface().has("create_goal"));
...
check("① MANAGED_CARRIER_TOOLS 覆盖 v0.9 workflow 与 kaz-context-policy 工具，MANAGED_PLUGINS 无 create-plan", MANAGED_CARRIER_TOOLS["ka-whale-workflow"]?.includes("whale_report") === true && MANAGED_CARRIER_TOOLS["ka-whale-workflow"]?.includes("ka_sub_whale") === true && JSON.stringify(MANAGED_CARRIER_TOOLS["kaz-context-policy"]) === JSON.stringify(["context_search", "context_read"]) && MANAGED_CARRIER_TOOLS["create-plan"] === undefined && !MANAGED_PLUGINS.some((p) => p.id === "create-plan"));
