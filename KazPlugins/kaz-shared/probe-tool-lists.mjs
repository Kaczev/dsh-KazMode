...
check("① 36.5 main Persona: delegation-first/plugin-preflight/maintenance routing", KAZ_ROLE_PROMPTS.main.includes("plugin-preflight (when needed)") && KAZ_ROLE_PROMPTS.main.includes("execute persona=main plan items on the main line") && KAZ_ROLE_PROMPTS.main.includes("delegate each persona=worker plan item individually via ka_sub_whale") && KAZ_ROLE_PROMPTS.main.includes("After working is complete, always enter memory-maintenance before any communication") && KAZ_ROLE_PROMPTS.main.includes("pre-finalize and delegate only pluginCreator items"));
check("① 36.8 main Persona 含 task splitting 与 pluginCreator 保留给 plugin-preflight", KAZ_ROLE_PROMPTS.main.includes("separate planItems per coherent task") && KAZ_ROLE_PROMPTS.main.includes("do not pack all work into one planItem") && KAZ_ROLE_PROMPTS.main.includes("pluginCreator is used only in plugin-preflight"));
