...
check("① KAZ_MAIN_ROLE_BODY 机制已退役且不再导出，main 是完整 Persona 首尾句", !Object.prototype.hasOwnProperty.call(await import("./lib/tool-lists.js"), "KAZ_MAIN_ROLE_BODY") && KAZ_ROLE_PROMPTS.main.startsWith("You are a helpful software engineer assistant. **ALWAYS REASON AS 'WE'**. Maintain a calm, declarative tone.") && KAZ_ROLE_PROMPTS.main.endsWith("The final white response should be crisp and to the point, and only appear after reasoning and working.") && KAZ_ROLE_PROMPTS.main.includes("During working, execute persona=main plan items on the main line"));
check("① 37.5 main Persona: delegation-first/maintenance routing without plugin-preflight", KAZ_ROLE_PROMPTS.main.includes("execute persona=main plan items on the main line") && KAZ_ROLE_PROMPTS.main.includes("delegate each persona=worker plan item individually via ka_sub_whale") && KAZ_ROLE_PROMPTS.main.includes("After working is complete, always enter memory-maintenance before any communication") && !KAZ_ROLE_PROMPTS.main.includes("plugin-preflight") && !KAZ_ROLE_PROMPTS.main.includes("pre-finalize and delegate only pluginCreator items"));
check("① 37.5 main Persona 含 task splitting 且不再把 pluginCreator 留给 plugin-preflight", KAZ_ROLE_PROMPTS.main.includes("separate planItems per coherent task") && KAZ_ROLE_PROMPTS.main.includes("do not pack all work into one planItem") && !KAZ_ROLE_PROMPTS.main.includes("pluginCreator is used only in plugin-preflight") && !KAZ_ROLE_PROMPTS.main.includes("pluginCreator planItems are reserved"));
...
check("① 36.8 worker Persona 不在 challenge/check-tools 提前报告工具不足", KAZ_ROLE_PROMPTS.subagent.worker.includes("The full working file-tool set (edit, write, pwsh, read) is granted in the working stage") && KAZ_ROLE_PROMPTS.subagent.worker.includes("do not report tool insufficiency before reaching working"));
check("① 三子代理 Persona 含 report 后硬停等/父回复恢复/terminal 新轮纪律", Object.values(KAZ_ROLE_PROMPTS.subagent).every((text) => text.includes("After calling your *_sub_whale_report, STOP and end the turn: do not call more tools or emit further output; wait for the parent main model's reply, which resumes you.") && text.includes("At the terminal communication stage, the next parent message starts a fresh delegation (assess-complexity / assess-delegation).")));
check("① main Persona working 段含父主侧 send_message 恢复/新轮纪律", KAZ_ROLE_PROMPTS.main.includes("Each *_sub_whale_report pauses the child until you reply") && KAZ_ROLE_PROMPTS.main.includes("send_message to resume it") && KAZ_ROLE_PROMPTS.main.includes("Do not assume the child keeps running after a report."));
