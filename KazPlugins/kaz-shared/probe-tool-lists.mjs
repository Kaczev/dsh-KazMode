...
check("① main 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.main.includes("Main whale role: drive the ka-whale-workflow run and verify delegated reports.") && KAZ_ROLE_PROMPTS.main.includes("Main flow: assess-complexity → challenge-plan → decide-tools → write-plan → decide-goal → working (or goal-active) → memory-maintenance → plugin-maintenance → communication.") && KAZ_ROLE_PROMPTS.main.includes("Manage context proactively: use context_search/read when earlier exact detail matters") && KAZ_ROLE_PROMPTS.main.includes("preview with context_compress suggest, then fold."));
check("① worker 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.worker.includes("Worker whale role: execute one delegated plan item, then stop after work_sub_whale_report and wait for the parent main model's reply.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Worker flow: assess-complexity → challenge-plan → check-tools → working → communication.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Each *_sub_whale_report pauses you until the parent replies; at terminal communication, the next parent message starts a fresh delegation.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Do not write memories or private plugins yourself."));
check("① memoryMaintainer 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Memory whale role: maintain project memories with evidence, keep new entries as CANDIDATE, delete only explicitly listed items with an audit record.") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Flow: assess-delegation → plan-memory → save-update or delete-memory → communication.") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Manage context proactively: use context_search/read"));
check("① pluginMaintainer 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Plugin whale role: maintain private plugins with CANDIDATE → implementation → probe → registration/versioning discipline, and report changed files, probe results, and rollback paths.") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Flow: assess-delegation → plan-plugin → create-plugin or update-plugin or retire-plugin → communication.") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Do not write memories.") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Manage context proactively: use context_search/read"));
