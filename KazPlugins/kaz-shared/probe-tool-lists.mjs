...
check("① v0.9 Stable Main Surface = 22（§1.1，M3.3 含 context 三工具，无 create_goal/subagent）", KAZ_STABLE_MAIN_TOOLS.length === 22 && KAZ_STABLE_MAIN_TOOLS.every((t) => typeof t === "string") && KAZ_STABLE_MAIN_TOOLS.includes("context_compress") && KAZ_STABLE_MAIN_TOOLS.includes("context_read") && KAZ_STABLE_MAIN_TOOLS.includes("context_search") && KAZ_STABLE_MAIN_TOOLS.includes("whale_report") && KAZ_STABLE_MAIN_TOOLS.includes("ka_sub_whale") && KAZ_STABLE_MAIN_TOOLS.includes("list_agents") && KAZ_STABLE_MAIN_TOOLS.includes("send_message") && KAZ_STABLE_MAIN_TOOLS.includes("interrupt_agent") && !KAZ_STABLE_MAIN_TOOLS.includes("create_goal") && !KAZ_STABLE_MAIN_TOOLS.includes("subagent"));
check("① stableMainSurface = v0.9 固定集 22", stableMainSurface().size === 22 && stableMainSurface().has("context_compress") && stableMainSurface().has("context_read") && stableMainSurface().has("context_search") && stableMainSurface().has("ka_sub_whale") && stableMainSurface().has("get_goal") && stableMainSurface().has("update_goal") && !stableMainSurface().has("exit_plan_mode") && !stableMainSurface().has("subagent") && !stableMainSurface().has("create_goal"));
...
check("① main 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.main.includes("Main whale role: drive the ka-whale-workflow run and verify delegated reports.") && KAZ_ROLE_PROMPTS.main.includes("Main flow: assess-complexity → challenge-plan → decide-tools → write-plan → decide-goal → working (or goal-active) → memory-maintenance → plugin-maintenance → communication.") && KAZ_ROLE_PROMPTS.main.includes("final report arrives as a single subagent-settled message") && KAZ_ROLE_PROMPTS.main.includes("reply once with send_message to resume it") && KAZ_ROLE_PROMPTS.main.includes("Manage context proactively: use context_search/read when earlier exact detail matters") && KAZ_ROLE_PROMPTS.main.includes("preview with context_compress suggest, then fold."));
check("① worker 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.worker.includes("Worker whale role: execute one delegated plan item, then call work_sub_whale_report (with nextStage when needed) to advance and set awaitingParent.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Do not call more tools; write your full report as your final message, end the turn, and wait for the parent main model's reply.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Worker flow: assess-complexity → challenge-plan → check-tools → working → communication.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("single settled channel") && KAZ_ROLE_PROMPTS.subagent.worker.includes("at terminal communication, the next parent message starts a fresh delegation.") && KAZ_ROLE_PROMPTS.subagent.worker.includes("是否复用由主代理决定：可对同 surface+空闲 child 直接 send_message，否则 ka_sub_whale 新开。") && KAZ_ROLE_PROMPTS.subagent.worker.includes("Do not write memories or private plugins yourself."));
check("① memoryMaintainer 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Memory whale role: maintain project memories with evidence, keep new entries as CANDIDATE, delete only explicitly listed items with an audit record.") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Flow: assess-delegation → plan-memory → save-update or delete-memory → communication.") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Use memory_sub_whale_report to advance, then do not call more tools; write your full report as your final message, end the turn, and wait for the parent main model's reply.") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("The parent receives it as subagent-settled") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("同一 memoryMaintainer 子代理可被多轮复用；每轮从 assess-delegation 开始，前一轮上下文仍在但本轮为独立委派。") && KAZ_ROLE_PROMPTS.subagent.memoryMaintainer.includes("Manage context proactively: use context_search/read"));
check("① pluginMaintainer 含角色流程一句 + context 纪律", KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Plugin whale role: maintain private plugins with CANDIDATE → implementation → probe → registration/versioning discipline, and report changed files, probe results, and rollback paths.") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Flow: assess-delegation → plan-plugin → create-plugin or update-plugin or retire-plugin → communication.") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Use plugin_maintainer_sub_whale_report to advance, then do not call more tools; write your full report as your final message, end the turn, and wait for the parent main model's reply.") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("The parent receives it as subagent-settled") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("是否复用由主代理决定：可对同 surface+空闲 child 直接 send_message，否则 ka_sub_whale 新开。") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Do not write memories.") && KAZ_ROLE_PROMPTS.subagent.pluginMaintainer.includes("Manage context proactively: use context_search/read"));
