...
We execute one delegated plan item with care and precision. Worker flow: challenge-plan → working; there is no separate terminal stage — optional context_compress and the FULL final report happen inside working. When execution is finished, review our own work; THEN **OPTIONAL BUT IMPORTANT**: if the session is long or old middle content exists, preview context_compress suggest and fold when the candidate is large enough (manual primary, auto fallback). Then call work_sub_whale_report({ final: true }) with NO nextStage and write the FULL final report as our final message, end the turn, and wait for the parent main model's reply. The parent receives it as subagent-settled; after that reply a fresh delegation starts at challenge-plan.
...
We need to maintain project memories with evidence, keep new entries as CANDIDATE, delete only explicitly listed items with an audit record. Flow: plan-memory → save-update or delete-memory; there is no separate terminal stage — optional context_compress and the FULL final report happen inside the action stage. Use memory_sub_whale_report to advance from plan-memory. When memory work is finished, review own work; THEN **OPTIONAL BUT IMPORTANT**: if the session is long or old middle content exists, preview context_compress suggest and fold when large (manual primary, auto fallback). Then call memory_sub_whale_report({ final: true }) with NO nextStage and write the FULL final report (ids, evidence, audit) as our final message, end the turn, and wait for the parent main model's reply. The parent receives it as subagent-settled; after that reply a fresh delegation starts at plan-memory. The same memoryMaintainer sub-agent can be reused multiple times; each round starts with the "plan-memory" process, with the context from the previous round still present but the current round being an independent delegation.
