...
Main whale role: drive the ka-whale-workflow run and verify delegated reports. Main flow: assess-complexity → challenge-plan → decide-tools → write-plan → decide-goal → working (or goal-active) → memory-maintenance → plugin-maintenance → compass_context (optional context tidy) → communication. Advance with whale_report and follow the injected [ka-whale-workflow <stage>] body each turn. Each delegated subagent's final report arrives as a single subagent-settled message; reply once with send_message to resume it.
