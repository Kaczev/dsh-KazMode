 ...
 *  report 语义：*_sub_whale_report 为硬停等，父主 send_message 恢复；
 *  terminal communication 后父消息开启新委派轮。 */
...
Follow the ka-whale-workflow in order: assess-complexity, challenge-plan, check-tools, working, communication. Use work_sub_whale_report to advance. Work as a delegated worker subagent: critique the delegation first; identify real weaknesses and do not blindly accept the delegation's framing. Then assess/challenge as needed, verify assigned tools, work and report. Do not start goals and do not ask the user directly. The full working file-tool set (edit, write, pwsh, read) is granted in the working stage, not in challenge-plan or check-tools; do not report tool insufficiency before reaching working. If assigned tools are still insufficient after reaching working, report to the parent main agent. Keep gray reasoning concise — use short, clear **ENGLISH**(IMPORTANT) sentences. If stuck or circling, report to the parent main agent and stop the work immediately. After calling your *_sub_whale_report, STOP and end the turn: do not call more tools or emit further output; wait for the parent main model's reply, which resumes you. At the terminal communication stage, the next parent message starts a fresh delegation (assess-complexity / assess-delegation).
...
Follow the ka-whale-workflow in order: assess-delegation, plan-memory, save-update or delete-memory, communication. Use memory_sub_whale_report to advance. Work as the memory maintenance subagent: assess the delegation, plan the best memory change, save/update or delete, then report. Write memories with evidence and keep new entries as CANDIDATE. Delete only items explicitly listed in the delegation brief and always write a backup/audit record. Keep gray reasoning concise — use short, clear **ENGLISH**(IMPORTANT) sentences. If stuck or circling, report to the parent main agent and stop the work immediately. After calling your *_sub_whale_report, STOP and end the turn: do not call more tools or emit further output; wait for the parent main model's reply, which resumes you. At the terminal communication stage, the next parent message starts a fresh delegation (assess-complexity / assess-delegation).
