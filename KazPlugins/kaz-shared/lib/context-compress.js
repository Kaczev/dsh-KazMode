...
// ===========================================================================
// 依据：不入库文件/Kaz7.0更新规划/最终基准 描述 Kaz7.0 上下文压缩.md（权威最终基准 v1.1）
// 职责（只做常量/归一化/分类/判据，绝不读写文件、不注册 cordis、不注入请求）：
//   * 升华阈值、渲染顺序、cache 可用性矩阵（A/B/C/D）；
//   * 原生 1M 窗口兜底策略（主动归档离根最近的旧高层概念块，非 token 预算）；
...
/** 冻结默认升华阈值：同层 ≥4 个兄弟触发父层摘要（v1.1 §5）。N 是结构数量阈值，不是 token 预算。 */
export const SUBLIMATION_THRESHOLD = 4;

/**
 * v1.1 §4.3 常驻内容渲染顺序（硬性）。
...
/** v1.1 §8.2 cache 可用性矩阵（硬门禁只对 A 类成立）。 */
...
/**
 * v1.1 §6.2 原生 1M 窗口兜底策略（极端意外情形）。
 * 策略只描述“主动归档并移除离根最近的（最老、最高层）概念块，直到显著低于原生窗口”，
 * 不冻结任何 MC/token 触发值或常驻保留预算。
 */
export const KAZ_CONTEXT_NATIVE_FALLBACK_STRATEGY = Object.freeze({
  id: "archive-nearest-root-outermost-oldest-highest",
  trigger: "active-tree-or-resident-content-approaches-native-window",
  target: "current-outermost-oldest-highest-block",
  action: "archive-block-details-first-then-remove-until-significantly-below-window",
  removalOrder: Object.freeze(["oldest", "highest-level", "nearest-root"]),
  archiveFirst: true,
  archiveStore: "archive-store-or-original-session-log",
...
/**
 * 按 usage 字段可用性分类到 A/B/C/D（v1.1 §8.2）。
...
/**
 * H_full = cacheRead / (uncached + cacheRead + cacheWrite)（v1.1 §8.1）。
...
/**
 * H_read_proxy = cacheRead / (uncached + cacheRead)（v1.1 §8.1，B 类降级代理）。
...
/**
 * v1.1 §10 C2 压缩效率判据（只作事后验收，不是触发/预算）：
