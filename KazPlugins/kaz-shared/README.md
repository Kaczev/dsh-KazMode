...
| `KAZ_ROLE_PROMPTS` | ka-whale-workflow / kaz-mode / kaz-system-prompt | Kaz7.0v2 §1 四角色 Persona 唯一收口（main + worker/memoryMaintainer/pluginMaintainer，骨架逐字 + 角色流程一句 + context 纪律，不含旧大段机制块）；主会话真实系统由 kaz-system-prompt 把 `deployment:persona` 设为 `KAZ_ROLE_PROMPTS.main` 全文；stage-defs 的 `V09_ROLE_PERSONAS` 与 main/subagent flow 文本由此派生 |
