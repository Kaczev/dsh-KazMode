...
> **官方/Kaz 分类修改点**：`lib/tool-plugin-catalog.js`。外置插件数据（手动添加 / 用户移除）保存在用户目录 storages 的 `other-*.json`，**不写在源码里**。

## 工具面语义（2026-08 统一）

- **官方/外置统一为“工具插件”**：Kaz 工具面 = `原设置(代码 + 用户 other-*) → 用户默认 → 项目设置`
  四文件合并后的 enabled 工具；新插件/新工具不会自动写入，需要手动添加（写入用户 `other-*`）。
