...
| `KAZ_ROLE_PROMPTS` | ka-whale-workflow / kaz-mode | v0.9 §9.1–9.5 全量 Persona 唯一收口（`main` + 四个 subagent 角色）；stage-defs 的 `V09_ROLE_PERSONAS` 与 main/subagent flow 文本由此派生；36.5 已同步 delegation-first（persona=main 主线执行 / subagent 委派）、plugin-preflight 与 maintenance 路由语义 |
