...
//   - Kaz Stable Main Surface 固定 21 项（含 plan_read / ask_user_question），含 context_compress/context_read/context_search，无 get_goal/update_goal/enable_tool / old subagent / create_goal；
...
const CONTROLLED_WORKER_SURFACE = [
  ...
  "pwsh", "read", "context_read", "context_search", "context_compress", "todo_write", "web_search", "write", "work_sub_whale_report", "job_list",
];
const kaWhaleMock = {
  stageOf: () => "working",
...
// ② Kaz 恒开：old memory-off state ignored.
{
  ...
  check("Kaz memory-off legacy state still yields memory/context reads (B5/M3.3 fixed)", nomemSurface !== null && nomemSurface.has("memory_search") && nomemSurface.has("memory_list") && nomemSurface.has("memory_detail") && nomemSurface.has("context_compress") && nomemSurface.has("context_read") && nomemSurface.has("context_search") && nomemSurface.size === 21);
}
...
const ALL_TOOLS = ["read", "pwsh", "web_search", "read_image", "job_list", "subagent", "enable_tool", "exit_plan_mode", "create_goal", "get_goal", "update_goal", "whale_report", "ka_sub_whale", "list_agents", "send_message", "interrupt_agent", "memory_search", "memory_save", "context_read", "context_search", "context_compress", "plan_read", "ask_user_question"];
...
check("assemble keeps v0.9 fixed surface only", assembled.has("ka_sub_whale") && assembled.has("whale_report") && assembled.has("context_compress") && assembled.has("context_read") && assembled.has("context_search") && assembled.has("plan_read") && assembled.has("ask_user_question") && !assembled.has("enable_tool") && !assembled.has("subagent") && !assembled.has("create_goal") && !assembled.has("get_goal") && !assembled.has("update_goal") && !assembled.has("read_image") && !assembled.has("job_list"));
