...
check("36.6 主 Persona 含事件驱动等待语义", MAIN_FLOW_TEXT.includes("must NOT use pwsh sleep") && MAIN_FLOW_TEXT.includes("poll list_agents") && MAIN_FLOW_TEXT.includes("end the current turn and wait for the subagent's report/finished message") && MAIN_FLOW_TEXT.includes("list_agents and send_message are not wait primitives")); check("主 Persona 不催促子代理尽快完成", MAIN_FLOW_TEXT.includes("Subagents follow their own staged workflow") && MAIN_FLOW_TEXT.includes("never urge immediate completion")); check("主 Persona 含任务拆分纪律", MAIN_FLOW_TEXT.includes("Before delegating, split work into small focused planItems") && MAIN_FLOW_TEXT.includes("never hand an entire phase or multi-part task to one subagent delegation"));
