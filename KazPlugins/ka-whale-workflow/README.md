...
- 补充信息：第 2、3、4……轮（turn≥2、模型不在运行）直接重新进入「任务重构」。**插话（模型运行中同一轮中途追加消息）不改变当前工作流阶段**。**Plan 模式激活时用户回复不进入任务重构**，保持在 plan 模式。
...
- 复盘（方向1）：任务进入 `done` 后，在 **Normal 任务完成**、**Plan 结束**、**Goal 结束**节点各注入一次 `[kaz-memory Review]` 紧凑指引（每 session 每类型一次）；指引要求模型只在有实质变化/新结论时写 1–2 条 `memory_save`，默认 `CANDIDATE`，无证据不得标 `high`，无实质结论不写。
