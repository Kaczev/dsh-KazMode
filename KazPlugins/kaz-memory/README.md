...
- **总开关（2026-08-21）**：settings.yaml `kaz-memory` 段的 `enabled`（默认
  ...
  按原有逻辑自动判断。
- **指引配置（2026-08-17）**：settings.yaml `kaz-memory` 段只剩两处生效——
  `guidance`（旧字段，整段覆盖，非空时完全取代固定提示，兼容保留）与
  `guidanceHead`（固定提示总述行覆盖，留空 = 内置默认）；`guidanceSearch` /
  `guidanceSave` / `guidanceList` / `guidanceForget` 保留兼容但**不再生效**（工具细节
  已并入各工具描述）。Kaz 模式面板的 kaz-memory 行提供 `enabled` 开关与
  `guidanceHead` 配置入口。
- **闸门不变**：模型 memory_save 仍然只能写 `suggested`，只有人在面板点「确认生效」
  才会置为 `auto`；「忽略」置为 `suggest`（参考，不注入）；「删除」即 forget。
...
3. 头部「记忆」按钮在 Kaz 按钮左侧；有 suggested 记忆时绿点 + 数字，点击弹出记忆面板：
...
6. 面板「确认生效」后该记忆 status 变 `auto`，`memory_list` 可查（只回 id/namespace/status/autoLoad/名称）。
