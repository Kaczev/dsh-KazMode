...
13. 面板打开期间发生 `memory_save` / `memory_update` 时，面板上方弹出带箭头指向面板的「记忆保存「标题」」/「记忆更新「标题」」气泡，自动淡入、停留后淡出；改名/自动载入/删除不触发。
