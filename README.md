...
- **绝不要**在仓库里运行 `git clean -fdx` 或 `git checkout -f`：`kaz\`、`test-kaz\` 都是仓库 ↔ live 预设的 junction，这类命令会顺着 junction 写坏 live 预设。仓库脏了用 `git status` / `git diff` 查看，只手动改需要的文件。
...
- **绝对不要**对仓库运行 `git clean -fdx` / `git checkout -f`：`kaz`、`test-kaz` 都是仓库 ↔ live 的 junction，会顺着写坏 live 预设。
