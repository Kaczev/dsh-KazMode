...
>
> **版本要求**：Kaz 预设形态**只支持 dsh `0.1.7-rc.2`**（第 0 步硬校验；不符合就停）。主环境 `.dsh` 的运行时是全局 `0.1.7-rc.2`，测试环境 `.dsh-test` 是它自己的本地副本 `0.1.7-rc.2`，干净环境 `.dsh-clean` 是**一个空白的 dsh**（不装 Kaz，用来对照"最干净、最不会出意外"的基线），它仍是自己的 `0.1.5-rc.2`——不在支持列表内，第 0 步会明确 `FAIL`。旧插件形态（dsh `0.1.1-rc.2`）见 **附录 A**。

---

## 第 0 步 版本闸门（硬性，必须先通过）

> 本版 Kaz 是 **DSH agent preset（预设形态）**，**只支持 dsh `0.1.7-rc.2`**。
...
> 本版 Kaz 是 DSH agent preset（预设形态），**只支持 dsh `0.1.7-rc.2`**，检测到本机运行时版本不在支持列表内。请先把 dsh 升级到 `0.1.7-rc.2` 再安装。若你还在用旧插件形态（dsh `0.1.1-rc.2`），先按本文件「附录 A」清理旧形态，升级 dsh 后再回到第 0 步。若用户明确要求把运行时**回退到 `0.1.5-rc.1`**：安装 / 更新必须显式加 `-SkipVersionCheck`，并且要把该 home 启动器的 `EXPECTED_CLI` 一起改回 `0.1.5-rc.1`，否则启动器开屏就拒绝启动——回退不是受支持状态，只能由用户决定。

> **三台 home 的闸门读到的都是自己的真实版本（已无「残留副本遮蔽」现象）**：第 0 步 / 第 2 步的候选顺序先读 `<home>\tools\dsh-cli\...`，再读 `<home>\profiles\<profile>\node_modules\@deepseek-ai\dsh\...`，最后读全局 `%APPDATA%\npm`，取第一个存在者即停。主环境 `.dsh` 前两个候选都不存在（那份 rc.1 残留副本已删除），读到**全局 `0.1.7-rc.2`**；`.dsh-test` 读到自己的本地副本 `0.1.7-rc.2`，二者都 `PASS`；`.dsh-clean` 读到自己 `tools\dsh-cli` 的 `0.1.5-rc.2`（**它是空白 dsh、没跟着升到受支持版本**），闸门**明确 `FAIL`**——这是改成只支持 `0.1.7-rc.2` 之后的预期结果，它本来也不装 Kaz。**看打印出来的路径判断闸门读的是哪一份**。
...
- **预期结果**（本机示例，2026-09-25 实测）：
  - `.dsh`（主环境，闸门落到**全局 `0.1.7-rc.2`**，打印的也是全局那条路径）→ `OK`。
  - `.dsh-clean`（**空白 dsh**，不装 Kaz；运行时仍是 `0.1.5-rc.2`）→ 报 **`FAIL`**（版本闸门；**改成只支持 `0.1.7-rc.2` 之后的预期结果**）。
  - `.dsh-test`（测试环境，本地副本 `0.1.7-rc.2`）→ 实测 **`FAIL`**，原因是 `multiple profiles under …: headless, web; pass -ProfileName`（它有 `headless` 与 `web` 两个 profile，`-AllHomes` 无法替你选）。**它 `FAIL` 与版本无关**——想让它过就在汇总表里给该 home 指定 `-ProfileName web`。
...
```powershell
Get-Content "$env:USERPROFILE\.dsh\.agent-presets\kaz\VERSION"                     # 应与仓库 kaz\VERSION 同一行
(Get-Item "$env:USERPROFILE\.dsh\.agent-presets\kaz\node_modules\@deepseek-ai").Target   # 应指向 <home>\profiles\node_modules\@deepseek-ai
```

- `VERSION` 与仓库 `kaz\VERSION` 不一致（或文件不存在）→ 镜像没到位：先 `Get-Content "<仓库路径>\kaz\VERSION"` 看发布源版本，再重跑安装程序；两处都对不上就报告用户 `kaz\VERSION` 的问题。
...
**出错处理**：
- 报 `VERSION GATE: FAIL` → 该 home 的运行时 dsh 不在支持列表内（只支持 `0.1.7-rc.2`）：**停下**，按第 0 步的说明转告用户；**不要**自行用 `-SkipVersionCheck` 绕过（唯一例外：用户明确要求回退到 `0.1.5-rc.1`，那时按第 0 步的说明加它）。
...
- **主代理工具面**（与官方标准预设对齐；完整清单以 `kaz/agent.cordis.yml` 与 `kaz-shared` 的黑名单为准）：
  ...
  - **技能**：`skill` 工具的目录里除官方技能外，还应看到 Kaz 自带的 **16 个**技能（如 `planning-with-files`、`cleaning-up-slop`、`kaz-dispatch`）；一个都没有 = 预设置的镜像源是**没有自带技能**的旧版（实测：`8.2.2` 时 12 份、`8.2.7` 起 18 份、`8.3.1` 起 19 份；`8.3.14` 合并到 14 份，`8.7.0` 起 16 份）。**注意主代理与子代理看到的份数不同**：9 份编排类技能（`building-something-new` / `working-from-a-plan` / `repairing-something-broken` / `reviewing-someone-elses-work` / `moving-or-upgrading-a-thing` / `getting-unstuck` / `orienting-in-a-codebase` / `cleaning-up-slop` / `kaz-dispatch`）只给主代理看，子代理的目录里少这 9 份是**正确的**，不是没装好。**加/减技能时要同时改这里**——名单在 `functions/kaz-shared/lib/skill-visibility.js`，而本文件与更新指引各有一处数字，改少了会把装好的环境判成坏的。
...
- **注入**：用户每发一条消息会看到 `[ka-whale-workflow idle]` 阶段注入（上下文注入，不是系统提示段）；上下文占用 ≥50% 时会出现 `[ka-context-policy compression-hint]` 提醒（**每个模型步检查一次，即一轮里每次工具调用一次，但只有真触发条件才发**：首次过 50%、比"上次提醒时"又涨 ≥5 个点、刚完成一次压缩（**每条压缩只反馈一次**）、或一直没压（**整段停留期只升级一次**为 `[ka-context-policy compression-hint · ACTION REQUIRED]`，升级措辞与压缩反馈不会并到同一条）。注：2026-09-13 之前的老版本在压缩过一次后会退化成**每步一条**，测试区已修）；idle 阶段连续调用观察类工具（`grep`/`glob`/`read`/`pwsh`）满 5 次会注入 `[ka-whale-workflow memory_hint]`；一旦一轮内工具调用总数到 32（之后每 +16）就会注入 `[ka-whale-workflow diving-hint]`，这条提示**同时报出本轮已经耗了多久**（从本轮那条用户消息算起，如 `This round has been running for 23 minutes.`）。
...
> 本附录只做**清理**，**不负责升级 dsh**。旧插件形态（≤ 7.4.x，dsh `0.1.1-rc.2`）与预设形态互斥；迁移顺序是：**先清理 → 再把 dsh 升级到 `0.1.5-rc.2` 以上（受支持的版本只有 `0.1.7-rc.2`；`0.1.5-rc.2` 的运行时已不被闸门接受，别停在它上面）→ 重启 → 回到第 0 步安装预设**。
...
7. **把 dsh 升级到 `0.1.7-rc.2`**（唯一受支持的版本），重启 `dsh web`，然后从本文件**第 0 步**开始安装预设。
...
| 现象 | 处理 |
| --- | --- |
| 第 0 步 `VERSION GATE: FAIL` | runtime dsh 不在支持列表内（只支持 `0.1.7-rc.2`）：**停止**，把第 0 步给用户的说明原样转达；旧形态先走附录 A |
| 第 2 步又报 `VERSION GATE: FAIL` | 目标 home 的运行时不在支持列表内（只支持 `0.1.7-rc.2`）——本机 `.dsh` 与 `.dsh-test` 都在支持列表内，所以这两个 home 正常情况下不该 FAIL；`.dsh-clean` 是空白 dsh、仍是 `0.1.5-rc.2`，预期 `FAIL`：换一台受支持的 home；**不要**自行用 `-SkipVersionCheck`（唯一例外：用户明确要求回退到 `0.1.5-rc.1`，见第 0 步） |
| `-AllHomes` 里某个 home 报 `FAIL` | **先看错误文字**：版本不符只是原因之一，多 profile 的 home 会因 `pass -ProfileName` 报 `FAIL`（本机 `.dsh-test` 就是这样，与版本无关）。本机现在只有 `.dsh`（全局 `0.1.7-rc.2`）与 `.dsh-test`（自己的本地副本 `0.1.7-rc.2`）在支持列表内；`.dsh-clean` 是空白 dsh、仍是 `0.1.5-rc.2`，**预期 `FAIL`**（它本来也不装 Kaz）。**不要**自行用 `-SkipVersionCheck`（唯一例外：用户明确要求回退到 `0.1.5-rc.1`，见第 0 步） |
