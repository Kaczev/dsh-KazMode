...
>
> **版本要求**：Kaz 预设形态**只支持 dsh `0.1.5-rc.2` 与 `0.1.7-rc.2`**（第 0 步硬校验；不符合就停）。主环境 `.dsh` 的运行时是全局 `0.1.5-rc.2`，测试环境 `.dsh-test` 是它自己的本地副本 `0.1.7-rc.2`，干净环境 `.dsh-clean` 是**一个空白的 dsh**（不装 Kaz，用来对照"最干净、最不会出意外"的基线），运行时同样是 `0.1.5-rc.2`。旧插件形态（dsh `0.1.1-rc.2`）见 **附录 A**。

---

## 第 0 步 版本闸门（硬性，必须先通过）

> 本版 Kaz 是 **DSH agent preset（预设形态）**，**只支持 dsh `0.1.5-rc.2` 与 `0.1.7-rc.2`**。
...
```powershell
$supportedDsh = @("0.1.5-rc.2", "0.1.7-rc.2")
$candidates = @(
...
> 本版 Kaz 是 DSH agent preset（预设形态），**只支持 dsh `0.1.5-rc.2` 与 `0.1.7-rc.2`**，检测到本机运行时版本不在支持列表内。请先把 dsh 升级到 `0.1.5-rc.2` 或 `0.1.7-rc.2` 再安装。若你还在用旧插件形态（dsh `0.1.1-rc.2`），先按本文件「附录 A」清理旧形态，升级 dsh 后再回到第 0 步。若用户明确要求把运行时**回退到 `0.1.5-rc.1`**：安装 / 更新必须显式加 `-SkipVersionCheck`，并且要把该 home 启动器的 `EXPECTED_CLI` 一起改回 `0.1.5-rc.1`，否则启动器开屏就拒绝启动——回退不是受支持状态，只能由用户决定。

> **三台 home 的闸门读到的都是自己的真实版本（已无「残留副本遮蔽」现象）**：第 0 步 / 第 2 步的候选顺序先读 `<home>\tools\dsh-cli\...`，再读 `<home>\profiles\<profile>\node_modules\@deepseek-ai\dsh\...`，最后读全局 `%APPDATA%\npm`，取第一个存在者即停。主环境 `.dsh` 前两个候选都不存在（那份 rc.1 残留副本已删除），读到**全局 `0.1.5-rc.2`**；`.dsh-test` 读到自己的本地副本 `0.1.7-rc.2`；`.dsh-clean` 读到自己 `tools\dsh-cli` 的 `0.1.5-rc.2`（**它是空白 dsh，已随其它 home 一起升到受支持版本**，所以同样 `PASS`）。**看打印出来的路径判断闸门读的是哪一份**。
...
**出错处理**：
- 报 `VERSION GATE: FAIL` → 该 home 的运行时 dsh 不在支持列表内（`0.1.5-rc.2` / `0.1.7-rc.2`）：**停下**，按第 0 步的说明转告用户；**不要**自行用 `-SkipVersionCheck` 绕过（唯一例外：用户明确要求回退到 `0.1.5-rc.1`，那时按第 0 步的说明加它）。
...
> 本附录只做**清理**，**不负责升级 dsh**。旧插件形态（≤ 7.4.x，dsh `0.1.1-rc.2`）与预设形态互斥；迁移顺序是：**先清理 → 再把 dsh 升级到 `0.1.5-rc.2` 或 `0.1.7-rc.2`（当前仅这两个版本受支持）→ 重启 → 回到第 0 步安装预设**。
...
7. **把 dsh 升级到 `0.1.5-rc.2` 或 `0.1.7-rc.2`**（当前仅这两个版本受支持），重启 `dsh web`，然后从本文件**第 0 步**开始安装预设。
...
| 现象 | 处理 |
| --- | --- |
| 第 0 步 `VERSION GATE: FAIL` | runtime dsh 不在支持列表内（`0.1.5-rc.2` / `0.1.7-rc.2`）：**停止**，把第 0 步给用户的说明原样转达；旧形态先走附录 A |
| 第 2 步又报 `VERSION GATE: FAIL` | 目标 home 的运行时不在支持列表内（`0.1.5-rc.2` / `0.1.7-rc.2`）——本机三台 home 都受支持，所以正常情况下不该 FAIL：改用受支持的 home；**不要**自行用 `-SkipVersionCheck`（唯一例外：用户明确要求回退到 `0.1.5-rc.1`，见第 0 步） |
| `-AllHomes` 里某个 home 报 `FAIL` | **先看错误文字**：版本不符只是原因之一，多 profile 的 home 会因 `pass -ProfileName` 报 `FAIL`（本机 `.dsh-test` 就是这样，与版本无关）。本机三台 home 运行时都在支持列表内（`.dsh` 与 `.dsh-clean` 是 `0.1.5-rc.2`，`.dsh-test` 是 `0.1.7-rc.2`），`.dsh-clean` 是空白 dsh、预期 `OK`。**不要**自行用 `-SkipVersionCheck`（唯一例外：用户明确要求回退到 `0.1.5-rc.1`，见第 0 步） |
