...
// ===========================================================================
// v0.9 §3–§6 表格的单一事实源：主模型与两类子代理使用英文 stage id，
// Allowed tools / Can advance to / Task 全部由 v0.9 表格定义。
// 本文件不依赖 cordis / dsh 服务，供 lib/index.js 与离线探针共用。
// ===========================================================================

import { KAZ_ROLE_PROMPTS, KAZ_V09_MAIN_TOOLS } from "../../kaz-shared/lib/tool-lists.js";
...
Read-only: never edit or git-write; only git status --porcelain, git diff --stat, rg/Select-String, node <probe>. Delivery gate (immutable, here only): S -> evidenceGate:false explicitly; M -> true when externally verifiable, else false; L -> true by default.`,
    ...
    "write-plan": {
      ...
      canAdvance: ["working", "memory-maintenance", "compass_context_before_communication", "communication"],
      task:`Finalize plan via whale_report(finalPlanPayload); use plan_read. One planItem per coherent task; do not pack all work into one. persona: main, worker, memoryMaintainer; required planItemId/persona/task; optional summary (one-line purpose), dependsOn (planItemIds this item depends on), targets (files/dirs/domains), verification (concrete checks), assignedTools. Invalid payloads are rejected with structured plan-item-invalid error; nothing is persisted and no item is silently dropped. Delegation/memory/reviewer/visual rules: README.`,
    },
    working: {
    ...
    "memory-maintenance": {
      ...
      canAdvance: ["communication", "write-plan", "compass_context_before_communication"],
      task:
...
/** lifecyclePath 注入：对应阶段已移除，恒为 false。 */
