...
//      这个最常见的拖延理由直接堵掉。
//   由此定下一条硬约束：**降噪的方向是"更少但更有信息量"，不是更密**。
//   提高提醒频率（例如把门槛调低）会让提示更像噪音，方向相反。
//
// 状态机的要害（下面每一条都是实现必须满足的约束，不是形容）：
//   * **"刚压过"是一次性事件，"够幅度"才算数**：跌幅要从"上一次压缩时的水位"算起，
//     并且必须**涨回**那个水位，下一次下跌才算新的压缩事件。只认"比上次检查低"会被每步的
//     占用抖动反复点亮（70↔55、50↔60 这种来回震荡会退化成每步一条）。
//   * **相邻两步不发两条**：判据是"上一次检查有没有发过通知"，不发按步号算
//     （步号要么跨期重置、要么要跨期保留，两条路都会误判相邻）。
//   * **升级整期只给一个名额**："被忽略够多次"与"停留够久"两条路共用一个名额，
//     并在两次升级之间留冷却——它们说的是同一句话，前后脚各发一条就是同一个意思重复两遍。
//     名额只在本停留期结束时归还：占用跌破阈值就是一期结束，之后重新涨到 50% 以上按新一期算。
//   * **期结束，停留计数与"待报道的压缩事件"一起归零**，否则跨期那一步的判断会失真
//     （每期归零的计数器不能拿来判断"与上一步相邻"；待报道的降幅只能被一条通知消费一次）。
//   * **"要求你去压"与"已经在压"要分开数**：压缩反馈是好消息，既不增加"未理会的要求数"，
//     也不能把它清零后让下一条从 1 数起。
//
// 测量记录不在本仓库内（脚本在 `不入库文件\kaz-hint-probe-20260913\` 下的
// `hint-regression.mjs` 与 `adversarial.mjs`）：那些数字**无法从本文件重新推出**，
// 任何再次引用它们的说法都要重跑一次脚本，别照抄。
//
